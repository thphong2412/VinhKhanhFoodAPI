...
namespace VinhKhanh.Pages
{
    public partial class MapPage : ContentPage
    {
        ...
        private const double HighlightsExpandedHeight = 560;
        private const double HighlightsCollapsedHeight = 132;
        ...
        private readonly List<LanguageOption> _allLanguageOptions = new();
        private List<LanguageOption> _filteredLanguageOptions = new();
        private string? _pendingDynamicLanguageCode;
    ...
    private const int MaxPinsToRenderOnEmulator = 32;
        ...
        private sealed class LanguageOption
        {
            ...
            public string DisplayName { get; init; } = "English (en)";
        }
            ...
            LocationPollingService locationPollingService, AudioQueueService audioQueue, PermissionService permissionService, VinhKhanh.Services.ApiService apiService, IAudioGenerator audioGenerator, RealtimeSyncManager realtimeSyncManager, IMapOfflinePackService mapOfflinePackService, IAudioService audioService)
        {
            ...
            try
            {
                ...
                _currentLanguage = NormalizeLanguageCode(savedLanguage);
            }
            catch
            {
                _currentLanguage = "vi";
        ...
        private async Task ShowPoiDetail(PoiModel poi, bool userInitiated = false, bool hydrateFromApi = true)
        {
            ...
            var preferredLanguage = _currentLanguage;

            // Load static text quickly before hydration logic finishes
            var localContent = await GetContentForLanguageAsync(poi.Id, preferredLanguage);

            LblPoiName.Text = localContent?.Title ?? poi.Name;

            if (userInitiated)
            {
                _ = TrackPoiEventAsync("poi_detail_open", poi.Id, $"\"trigger\":\"tap\",\"lang\":\"{NormalizeLanguageCode(preferredLanguage)}\"");
            }

            // Start API Hydration without blocking UI updates.
            _ = Task.Run(async () =>
            {
                try
                {
                    ...
                    var dynamicUi = await BuildDynamicUiTextAsync(preferredLanguage);

                    var currentContent = await GetContentForLanguageAsync(poi.Id, preferredLanguage);
                    ...
                    {
                        LblPoiName.Text = currentContent?.Title ?? poi.Name;

                        try
                        {
                            var phone = currentContent?.PhoneNumber;
                            var addr = currentContent?.Address;
                            if (string.IsNullOrEmpty(phone) || string.IsNullOrEmpty(addr))
                            {
                                var fallback = await GetStrictContentForLanguageAsync(poi.Id, NormalizeLanguageCode(preferredLanguage));
                                if (fallback != null)
                                {
                            ...
                            try
                            {
                                if (LblPoiCategory != null) LblPoiCategory.Text = await LocalizeFreeTextAsync(poi.Category, preferredLanguage);
